...
export function useWellBeing() {
    ...
    const loadData = async () => {
      try {
        ...
        if (!cancelled) {
          ...
          if (firstCategoryKey) {
            dispatch({ type: 'SET_CURRENT_SELECTION', payload: { categoryKey: firstCategoryKey, questionIndex: 0 } });
          }
        }
      } catch (err) {
  ...
  const { categories, answers, currentCategoryKey, currentQuestionIndex } = state;
  ...
  const onSubmit = async () => {
    ...
    try {
      const categoryScores = Object.keys(categories).map(catKey => {
        ...
        const sum = qs.reduce((s, q) => s + (answers?.[catKey]?.[q.id] ?? 0), 0);
  ...
  const handleSelectQuestion = (categoryKey, questionIndex) => {
    dispatch({ type: 'SET_CURRENT_SELECTION', payload: { categoryKey, questionIndex } });
  };
  ...
  const navigateQuestion = (direction) => {
    ...
    if (nextIndex < 0 && direction < 0) {
      ...
      if (prevCategoryIndex >= 0) {
        ...
        handleSelectQuestion(categoryKeys[prevCategoryIndex], prevCategory.questions.length - 1);
      }
      return;
    }
